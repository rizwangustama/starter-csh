<template>
    <section>
        <div class="header-primary-overlay relative container-2">
            <div class="menu-wrap flex items-center text-blue justify-between mx-auto text-lg py-1 px-3 lg:py-0 lg:pr-0 lg:pl-5"
            :class="showDropdown ? 'bg-white':'bg-transparent'">
                <div class="space-y-2">
                ...
                <nav class="right-navigation" v-if="$settings.primary_menu" >
                    <div class="flex items-center justify-end gap-5 lg:gap-8">
                        ...
                        <div class="flex items-center justify-end gap-x-2">
                            <button class="hidden lg:inline-flex btn btn-primary h-[70px] rounded-none px-10 py-[23px]">Sign Up</button>
                            ...
                            <button title="open navigation" class=" md:hidden open-navigation" v-if="showDropdown" @click="showDropdown = !showDropdown" >
                                <svg-vue
                                    icon="menu-outline"
                                    width="24"
                                    height="24"
        ...
        <div :class="{ active: showDropdown }" class="menu-mobile">
            <div class="container py-16" v-if="$settings.primary_menu">
                <ul class="flex flex-col py-20 text-white items-center text-sm tracking-wide md:flex gap-5 lg:gap-8" >
                    <li v-for="item in $settings.primary_menu" :key="item.id">
...
header#vue-header.template-home-style-two {
    @apply w-full top-[70px] lg:top-[100px] absolute;
}
...
.menu-mobile {
    @apply fixed top-0 right-0 w-full h-screen bg-accent;
    z-index: 70;
    ul {
        @apply list-none;
        li {
